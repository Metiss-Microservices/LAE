...
class BroadcastRecipient(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    campaign_id = Column(
        UUID(as_uuid=True), ForeignKey("broadcast_campaigns.id")
    )
...
class Supplier(Base):
    ...
    score = Column(Float, default=5)
    wins = Column(Integer, default=0)
    rating_count = Column(Integer, default=0)
    completed_jobs = Column(Integer, default=0)
    cancelled_jobs = Column(Integer, default=0)
    response_speed = Column(Integer, default=60)
    monthly_free_credit_used = Column(Integer, default=0)
    last_free_credit_at = Column(DateTime, nullable=True)
    is_verified = Column(Boolean, default=False)
    is_active = Column(Boolean, default=True)
    is_blocked = Column(Boolean, default=False)
    telegram_chat_id = Column(String)
    bale_chat_id = Column(String)
    rubika_chat_id = Column(String)
    created_at = Column(DateTime, default=now)
...
class SupplierSubCategory(Base):
    ...
    subcategory_id = Column(
        UUID(as_uuid=True), ForeignKey("subcategories.id")
    )
    created_at = Column(DateTime, default=now)
...
class Lead(Base):
    ...
    expected_budget = Column(Float, default=0)
    priority_mode = Column(String, default="smart")
...
class LeadMatch(Base):
    ...
    status = Column(String, default="pending")
    bid_price = Column(Float, default=0)
    created_at = Column(DateTime, default=now)
...
class RaceLock(Base):
    ...
    locked_at = Column(DateTime, default=now)
...
class PaymentTransaction(Base):
    ...
    authority = Column(String, unique=True)
