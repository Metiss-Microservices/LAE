...
from routers.auth import get_supplier_by_token
from scoring.service import calculate_supplier_reputation
from services.bidding_service import place_bid
from services.claim_service import claim_lead
...
# LOCATION HELPER
# =========================================================
def get_location_name(db: Session, location_id):
    ...
    if loc.type == "district":
        parent = db.query(Location).filter_by(id=loc.parent_id).first()

        if parent:
            return f"{parent.name} / {loc.name}"
...
def update_profile(
    ...
    supplier.full_name = payload.get("full_name", supplier.full_name)
    supplier.location_id = payload.get("location_id", supplier.location_id)

    services = payload.get("services")

    if services is not None:
        db.query(SupplierCategory).filter_by(
...
def live_leads(token: str = Header(None), db: Session = Depends(get_db)):
    ...
    matches = (
        db.query(LeadMatch).filter_by(supplier_id=supplier_id).all()
    )
    ...
    for match in matches:
        ...
        lead = db.query(Lead).filter_by(id=match.lead_id).first()

        if not lead:
            continue

        category = db.query(Category).filter_by(id=lead.category_id).first()
        subcategory = (
            db.query(SubCategory).filter_by(id=lead.subcategory_id).first()
        )
        ...
        result.append(
            {
                ...
                "location": get_location_name(db, lead.location_id),
                "score": match.final_score,
                "bid_price": match.bid_price,
                "expires_at": match.expires_at,
                "priority_mode": lead.priority_mode,
            }
        )
...
def bid(
    ...
    if not lead_id:
        return {"success": False, "error": "missing_lead_id"}

    return place_bid(db, supplier_id, UUID(lead_id), bid_price)
...
async def claim(
    ...
    if not match_id:
        return {"success": False, "error": "missing_match_id"}

    return await claim_lead(db=db, supplier_id=supplier_id, match_id=UUID(match_id))
...
def won_leads(token: str = Header(None), db: Session = Depends(get_db)):
    ...
    matches = (
        db.query(LeadMatch)
        .filter_by(supplier_id=supplier_id, status="won")
        .order_by(LeadMatch.won_at.desc())
        .all()
    )
    ...
    for match in matches:
        ...
        client = db.query(Client).filter_by(id=lead.client_id).first()
...
def lead_details(
    ...
    match = (
        db.query(LeadMatch)
        .filter_by(lead_id=lead.id, supplier_id=supplier_id)
        .first()
    )
...
def race_status(
    ...
    match = (
        db.query(LeadMatch)
        .filter_by(id=UUID(match_id), supplier_id=supplier_id)
        .first()
    )
...
def auction_status(
    ...
    matches = db.query(LeadMatch).filter_by(lead_id=UUID(lead_id)).all()
...
def contact_info(
    ...
    match = (
        db.query(LeadMatch)
        .filter_by(lead_id=UUID(lead_id), supplier_id=supplier_id, status="won")
        .first()
    )

    if not match:
        return {"success": False, "error": "not_winner"}
...
def statistics(token: str = Header(None), db: Session = Depends(get_db)):
    ...
    services_count = (
        db.query(SupplierCategory).filter_by(supplier_id=supplier_id).count()
    )

    total_matches = (
        db.query(LeadMatch).filter_by(supplier_id=supplier_id).count()
    )
    ...
    total_lost = (
        db.query(LeadMatch)
        .filter_by(supplier_id=supplier_id, status="lost")
        .count()
    )

    total_pending = (
        db.query(LeadMatch)
        .filter_by(supplier_id=supplier_id, status="pending")
        .count()
    )
    ...
    return {
        ...
        "stats": {
            ...
            "win_rate": (
                round((total_wins / total_matches) * 100, 2)
                if total_matches
                else 0
            ),
        },
...
def dashboard(token: str = Header(None), db: Session = Depends(get_db)):
    ...
    reviews_count = (
        db.query(SupplierReview).filter_by(supplier_id=supplier_id).count()
    )
...
def notification_settings(
    ...
    supplier.notify_sms = payload.get("notify_sms", supplier.notify_sms)
    supplier.notify_telegram = payload.get(
        "notify_telegram", supplier.notify_telegram
    )
    supplier.notify_bale = payload.get("notify_bale", supplier.notify_bale)
    supplier.notify_rubika = payload.get(
        "notify_rubika", supplier.notify_rubika
    )
...
def channels(token: str = Header(None), db: Session = Depends(get_db)):
    ...
    return {
        "success": True,
        "channels": {
            "telegram": supplier.telegram_chat_id,
            "bale": supplier.bale_chat_id,
            "rubika": supplier.rubika_chat_id,
        },
    }
