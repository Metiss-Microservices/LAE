...
def get_claim_cost(
    ...
    pricing = CLAIM_PRICING.get(lead_type)

    if pricing is None:
        pricing = CLAIM_PRICING["service"]

    return pricing.get(
        priority,
        pricing.get("medium", 2),
    )
...
def get_pack_credits(pack_name):
